	...
	int main()
	{
		...
		for (int i = 0; i < sk; i++) {
			...
			if (randpaz == 0) {
				...
				Eilute_mas[i].vidurkis = Eilute.rez * 0.4 + Eilute_mas[i].Egzaminas * 0.6;
				cout << "Galutinis Rezultatas: " << fixed << setprecision(2) << Eilute_mas[i].vidurkis << "\n";

				cout << "\nMediana = " << fixed << setprecision(2) << findMedian(Eilute.Pazymiai, n);
				Eilute_mas[i].mediana = findMedian(Eilute.Pazymiai, n) * 0.4 + Eilute_mas[i].Egzaminas * 0.6;
				cout << "\nGalutinis rezultatas: " << fixed << setprecision(2) << Eilute_mas[i].mediana << "\n";
				Eilute.rez = 0;
				eil_vect.push_back(Eilute_mas[i]);
		}
			...
			if (Eilute.pasirinkimas == 0) {
				...
				for (int j = 0; j < sk; j++)
				{
					cout << left
						...
						<< setw(15) << eil_vect[j].mediana << endl;
				}
			}
			return 0;
	}
